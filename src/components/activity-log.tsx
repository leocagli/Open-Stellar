    ...
    <div className="flex flex-col border-t border-border bg-card/30">
      ...
      <div className="h-32 overflow-auto p-3 lg:h-40">
        <div className="flex flex-col gap-0.5">
          {logs.map((log, i) => (
            <p key={`${log}-${i}`} className={cn(
              "text-[11px] font-mono leading-relaxed",
              log.includes("ERROR") ? "text-[hsl(var(--status-error))]" :
              log.includes("SYSTEM") ? "text-primary" :
              log.includes("completed") || log.includes("recovered") ? "text-[hsl(var(--status-active))]" :
              "text-muted-foreground"
            )}>{log}</p>
          ))}
          {logs.length === 0 && <p className="text-[11px] font-mono text-muted-foreground italic">Waiting for activity...</p>}
        </div>
      </div>
    </div>
  )
